    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gerenciamento de contas e empresas</p>
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredContas.map((conta: any) => (
                    <tr key={conta.id} className="border-b border-border hover:bg-[#333333]/50">
                      <td className="py-3 px-4">
                        <p className="font-medium">{conta.nome}</p>
                      </td>
                      <td className="py-3 px-4 text-muted-foreground">{conta.email || "-"}</td>
                      <td className="py-3 px-4 text-muted-foreground">{conta.telefone || "-"}</td>
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${
                            statusColors[conta.status as keyof typeof statusColors] || "bg-[#333333] text-foreground/80"
                          }`}
                        >
